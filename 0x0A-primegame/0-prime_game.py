...
def primeNumbers(n):
    """
    Returns the list of prime numbers up to n using the
...
def isWinner(x, nums):
    ...
    :param nums: A list of integers representing the upper
        limits for each round.
    :return: The name of the winner ('Maria' or 'Ben') or
        None if there is no winner.
    1. Maria starts first and removes a prime number from
        the list.
    2. Ben then removes a prime number from the list.
    3. The player who cannot make a move loses.
    4. If both players can make moves, the game continues until
        all primes are removed.
    5. If Maria has more rounds won than Ben, she is the winner.
    6. If Ben has more rounds won than Maria, he is the winner.
    7. If both have the same number of rounds won, there is no
        winner.
    :rtype: str or None
    """
